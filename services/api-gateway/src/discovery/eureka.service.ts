...
export class EurekaService implements OnModuleInit, OnModuleDestroy {
    ...
    onModuleInit() { this.client.start(); }
    onModuleDestroy() { this.client.stop(); }

    getInstances(appId: string): EurekaClient.EurekaInstanceConfig[] {
        return (this.client.getInstancesByAppId((appId || '').toUpperCase()) as any) || [];
    }

    async waitForInstances(appId: string, timeoutMs = 5000): Promise<EurekaClient.EurekaInstanceConfig[]> {
        const end = Date.now() + timeoutMs;
        while (Date.now() < end) {
            const list = this.getInstances(appId);
            if (list.length > 0) return list;
            await new Promise(r => setTimeout(r, 400));
            try { await (this.client as any).fetchRegistry(); }
            catch (err: any) { console.warn('[eureka] fetchRegistry failed:', err?.message); }
        }
        return [];
    }
}
