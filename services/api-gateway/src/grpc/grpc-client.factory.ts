...
function resolveHost(inst: any) { return inst?.ipAddr || inst?.hostName || 'localhost'; }
...
export class GrpcClientFactory {
    private readonly waitTimeoutMs = Number(process.env.EUREKA_WAIT_TIMEOUT_MS ?? 10000);
    private readonly protosDir = process.env.PROTO_ROOT || join(process.cwd(), 'protos');
    ...
    private async waitForInstances(appName: string): Promise<any[]> {
        ...
        while (Date.now() < end) {
            ...
            try { await (this.eureka as any).client?.fetchRegistry?.(); } catch {}
        }
        return [];
    }
    ...
    async forService(appName: string, pkg: string, protoFile: string): Promise<ClientGrpc> {
