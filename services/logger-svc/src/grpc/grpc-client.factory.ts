...
function hostOf(inst: any) { return inst?.ipAddr || inst?.hostName || 'localhost'; }
function portOf(p: any) { return typeof p === 'number' ? p : (p && typeof p.$ === 'number' ? p.$ : undefined); }
function resolvePort(inst: any) {
    const p = portOf(inst?.port) ?? portOf(inst?.securePort);
    if (typeof p !== 'number') throw new Error('Eureka instance without port'); return p;
}
...
export class GrpcClientFactory {
    ...
    constructor(private readonly eureka: EurekaDiscovery, private readonly rr: RoundRobin) {}
    ...
    private async pick(appName: string) {
        ...
        while (Date.now() < end) {
            ...
            try { await (this.eureka.client as any).fetchRegistry?.(); } catch {}
        }
