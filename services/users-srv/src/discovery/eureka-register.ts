...
/**
 * Registra el microservicio de usuarios en Eureka.
 * Variables soportadas (prioridad):
 *  - Nombre: USERS_APP_NAME -> APP_NAME -> 'USERS-SERVICE'
 *  - Puerto gRPC: USERS_GRPC_PORT -> GRPC_PORT -> 50057
 *  - Host publicado: SERVICE_REGISTER_HOST -> REGISTER_HOST -> '127.0.0.1'
 */
export function registerInEureka() {
    const app = process.env.USERS_APP_NAME || process.env.APP_NAME || 'USERS-SERVICE';

    const host = process.env.SERVICE_REGISTER_HOST || process.env.REGISTER_HOST || '127.0.0.1';

    const port = Number(
        process.env.USERS_GRPC_PORT ||
