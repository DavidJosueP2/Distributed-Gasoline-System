 ...
 *  - Nombre: VEHICLES_APP_NAME -> APP_NAME -> 'VEHICLES-SERVICE'
 *  - Puerto gRPC: VEHICLES_GRPC_PORT -> GRPC_PORT -> 50051
 *  - Host publicado: SERVICE_REGISTER_HOST -> REGISTER_HOST -> '127.0.0.1'
 */
export function registerInEureka() {
    const app = process.env.VEHICLES_APP_NAME || process.env.APP_NAME || 'VEHICLES-SERVICE';
    ...
    const port = Number(
        process.env.VEHICLES_GRPC_PORT ||
